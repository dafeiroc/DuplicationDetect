 ...
 */
public class HashSequence extends Configured implements Tool {
    private static final Logger log = Logger.getLogger(HashSequence.class);
    ...
    public int run(String[] args) throws IOException {
        if (args.length < 2) {
            System.out.println("HashSequence <input-dir> <output-dir>  [mappers] [reducers]");
        ...
        JobConf conf = new JobConf(getConf(), HashSequence.class);
        FileSystem fs = FileSystem.get(conf);
        ...
        conf.setJobName("merge small file to hash value sequence file");
        conf.setInputFormat(WholeFileInputFormat.class);
        conf.setOutputFormat(SequenceFileOutputFormat.class);
        conf.setOutputKeyClass(Text.class);
        conf.setOutputValueClass(Text.class);
        conf.setMapperClass(HashSequenceMapper.class);
    ...
    public static void main(String[] args) throws Exception {
        int exitCode = ToolRunner.run(new HashSequence(), args);
